...
from utils_incremental import modified_resnet
from utils_incremental import modified_linear
import utils_incremental.utils_pytorch as utils_pytorch
...
test_batch_size = 128  # Batch size for test
...
for iteration_total in range(args.nb_runs):
    ...
    for iteration in range(start_iter, int(args.num_classes / args.nb_cl)):
        ...
        indices_train_10 = np.array(
            [i in order[range(last_iter * args.nb_cl, (iteration + 1) * args.nb_cl)] for i in Y_train_total])
        indices_test_10 = np.array(
            [i in order[range(last_iter * args.nb_cl, (iteration + 1) * args.nb_cl)] for i in Y_valid_total])
        ...
        ##############################################################
        ckp_name = ROOT_SAVE+'./checkpoint/{}_run_{}_iteration_{}_model.pth'.format(args.ckp_prefix, iteration_total, iteration)
        ...
        if args.resume and os.path.exists(ckp_name):
            ...
            if args.less_forget and args.mr_loss:
                ...
                tg_model = incremental_train_and_eval_AMR_LF(args.epochs, tg_model, ref_model, tg_optimizer,
                                                             tg_lr_scheduler, \
                                                             trainloader, testloader, \
                                                             iteration, start_iter, \
                                                             cur_lamda, \
                ...
                if args.less_forget:
                    ...
                    tg_model = incremental_train_and_eval_LF(args.epochs, tg_model, ref_model, tg_optimizer,
                                                             tg_lr_scheduler, \
                                                             trainloader, testloader, \
                                                             iteration, start_iter, \
                                                             cur_lamda)
                    ...
                    if args.mimic_score:
                        ...
                        tg_model = incremental_train_and_eval_MS(args.epochs, tg_model, ref_model, tg_optimizer,
                                                                 tg_lr_scheduler, \
                                                                 trainloader, testloader, \
                                                                 iteration, start_iter,
                                                                 args.lw_ms)
                        ...
                        tg_model = incremental_train_and_eval(args.epochs, tg_model, ref_model, tg_optimizer,
                                                              tg_lr_scheduler, \
                                                              trainloader, testloader, \
                                                              iteration, start_iter,
                                                              args.T, args.beta)
        ...
        torch.save(class_means, \
                   './checkpoint/{}_run_{}_iteration_{}_class_means.pth'.format(args.ckp_prefix, iteration_total,
                                                                                iteration))
        ...
        #     compute_confusion_matrix(tg_model, zeroth_model.cuda(), zero_evalloader)
        # else:
        #     zeroth_model = tg_model  # .cuda()
        #     zero_evalloader = evalloader
