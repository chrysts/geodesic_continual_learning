...
from algorithms.nce_loss import NTXentLoss
...
def incremental_train_and_eval_LF_nce(epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if iteration == start_iteration:
                ...
                # loss1 = nn.CosineEmbeddingLoss()(cur_features, ref_outputs.detach(), \
                #     torch.ones(inputs.shape[0]).to(device)) * lamda
                #loss1 = nce_criterion.calc_loss(cur_features, ref_features.detach(), batch_size=cur_features.size(0))*lamda
                loss1 = nce_criterion.calc_loss_label(cur_features, ref_features.detach(), targets, max_class=outputs.size(-1),
                                                      batch_size=outputs.size(0))*lamda
