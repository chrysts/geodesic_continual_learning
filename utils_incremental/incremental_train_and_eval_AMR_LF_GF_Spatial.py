...
def spatial_gfk_loss(feat_1, feat_2):
    loss = 0.
    for a, b in zip(feat_1, feat_2):
...
def incremental_train_and_eval_AMR_LF_GF(epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
    ...
    clr_inc = 2
    if iteration > start_iteration:
        total_classes = tg_model.fc.fc1.out_features + tg_model.fc.fc2.out_features
    else:
        total_classes = tg_model.fc.out_features

    #scheduled_factor = math.sqrt((iteration)*2 /25 )  ## for 25 phases
    #scheduled_factor = math.sqrt(lamda/8)   ## for 10 phases (total_classes) / clr_inc
    scheduled_factor = lamda#math.sqrt((iteration - start_iteration) / 1.) ## 4 atau 5 bagus
    L3_lambda = 4.*scheduled_factor+0. ### GOLDEN: 5, 10.5 dibagi 6    #L3_lambda *scheduled_factor + 3.

    #Lamda:  0.01 1.0simon123
    print('Lamda: ', L1_lambda, L3_lambda)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            curr_allfeatures, curr_features, outputs = tg_model(inputs, is_feature=True)
            if iteration == start_iteration:
                loss = nn.CrossEntropyLoss(weight_per_class)(outputs, targets)
            else:
                old_allfeatures, ref_features, ref_outputs = ref_model(inputs, is_feature=True)
                ...
                #loss3 = spatial_gfk_loss(old_allfeatures, curr_allfeatures)*L3_lambda*scheduled_factor
                loss3 = gfk.fit(selected_ref_features.detach(), selected_curr_features)*L3_lambda
                ...
                #     loss1 = nn.MarginRankingLoss(margin=dist)(gt_scores.view(-1, 1), \
                #         hard_scores.view(-1, 1), torch.ones(hard_num*K).to(device)) * lw_mr
                # else:
                #     loss1 = torch.zeros(1).to(device)
                ################################################
                loss = loss2 + loss3#loss1 + loss2 + loss3
            loss.backward()
            tg_optimizer.step()
