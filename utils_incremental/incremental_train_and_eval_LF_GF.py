...
from algorithms.GFK_distill  import GFK
...
def incremental_train_and_eval_LF_GF(epochs, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            cur_features, outputs = tg_model(inputs, is_feature=True)
            if iteration == start_iteration:
                ...
                loss2 = F.cross_entropy(outputs, targets)#nn.CrossEntropyLoss(weight_per_class)(outputs, targets)
                if epoch > epoch_min:
                    loss1 = gfk.fit(ref_features.detach(), cur_features) * lamda
                    loss = loss1 + loss2
                else:
                    loss = loss2
